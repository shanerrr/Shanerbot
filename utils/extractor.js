    ...
    return new Promise(async (resolve, reject) => {
      try {
          ...
          if (info.type === "track") {
            const track = {
              ...
              author: info.publisher
                ? info.publisher.name ??
                  info.publisher.artist ??
                  info.publisher.writer_composer ??
                  null
                : null,
              description: "",
              url: info.permalink,
            ...
            const tracks = await trackList.map((track) => {
              return {
                ...
                views: 0,
                author: track.publisher
                  ? track.publisher.name ??
                    track.publisher.artist ??
                    track.publisher.writer_composer ??
                    null
                  : null,
                description: "",
                url: track.permalink,
        ...
        if (
          ...
          query.includes("play.spotify.com")
        ) {
          ...
          if (info.type === "track") {
            ...
            const track = {
              ...
              async engine() {
                return (
                  await playdl.stream(
                    await Youtube.search(`${info.artist} ${info.title} lyric`, {
                      limit: 1,
                      type: "video",
                      safeSearch: true,
                    }).then((x) =>
                      x[0]
                        ? `https://youtu.be/${x[0].id}`
                  ...
                  return (
                    await playdl.stream(
                      await Youtube.search(
                        `${track.artists[0].name} ${track.name} lyric`,
                        { limit: 1, type: "video", safeSearch: true }
                      ).then((x) =>
                        x[0]
                          ? `https://youtu.be/${x[0].id}`
                          : `https://youtu.be/Wch3gJG2GJ4`
        ...
        if (
          ...
          const track = {
            ...
            async engine() {
              return (
                await playdl.stream(`https://youtu.be/${info[0].id}`, {
                  discordPlayerCompatibility: true,
                })
              ).stream;
            },
          ...
          const tracks = trackList.map((track) => {
            return {
              ...
              async engine() {
                return (
                  await playdl.stream(
                    await Youtube.search(track.url, {
                      limit: 1,
                      type: "video",
                      safeSearch: true,
                    }).then((x) =>
        ...
        if (search && search.length) {
          const tracks = search.map((track) => {
            return {
              ...
              async engine() {
                return (
                  await playdl.stream(`https://youtu.be/${track.id}`, {
                    discordPlayerCompatibility: true,
                  })
                ).stream;
              },
        ...
        console.log(
          `Extractor: An error occurred while attempting to resolve ${query} :\n${error}`
        );
        return resolve({ playlist: null, info: null });
      }
    });
